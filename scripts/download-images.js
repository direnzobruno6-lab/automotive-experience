...
async function downloadImage(car) {
    ...
    return new Promise((resolve, reject) => {
        ...
        https.get(car.url, options, (response) => {
            if (response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
                // Handle redirect manually if needed, though most browsers/libs handle it. 
                // Simple https.get doesn't auto-redirect, so we might need to recurse.
                // However, Wikimedia thumbs usually are direct or 301. Let's assume direct for now or use a recursive function.
                // Actually, simple way:
...
async function downloadAll() {
    for (const car of cars) {
        try {
            ...
            await sleep(2000); // 2 seconds delay to be polite and avoid 429
        } catch (error) {
